...
export const updateUserPreferences = async (preferences) => {
  try {
    const response = await apiClient.put(`${USER_BASE_PATH}/me/preferencias/`, preferences);
    return response.data;
  } catch (error) {
    const errorMessage = getErrorMessage(error, 'Error al actualizar las preferencias.');
    throw new Error(errorMessage);
  }
};